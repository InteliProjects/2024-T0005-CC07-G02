// Indica o uso em um ambiente do cliente (navegador) em vez de um servidor
"use client"

// Importa a biblioteca React Loading Skeleton para mostrar placeholders de carregamento
import Skeleton from 'react-loading-skeleton'
import 'react-loading-skeleton/dist/skeleton.css'
// Importa os componentes de card e dialog de ações de outros planos
import { Card, CardHeader, CardTitle, CardContent } from "@/components/ui/card";
import DialogAcoesOutros from "./dialogAcoesPlanoOutros";
// Importa o tipo (interface) Plano, para tipagem dos planos de outros planos
import { Plano } from "@/types/plano";
// Importa a biblioteca React para a criação de componentes
import React from 'react';

// Define as propriedades do componente CardPlanosTvEStreaming
interface CardPlanosTvEStreamingProps {
    planosContratados: Plano[];
    carregandoPlanosContratados: boolean;
}

// Componente funcional CardPlanosTvEStreaming que aceita as propriedades definidas acima 
export default function CardPlanosTvEStreaming({ planosContratados, carregandoPlanosContratados }: CardPlanosTvEStreamingProps)  {
    if ((planosContratados === undefined || planosContratados.length === 0) && carregandoPlanosContratados === false) {
        return (
                    // Cria uma grade para organizar os cartões dos planos
            <div className="flex justify-center items-center ml-2">
                <h1>Esse cliente não tem planos contratados</h1>
            </div>
        );
    }
    return (
            ...
            {(carregandoPlanosContratados === false) ? (
                ...
                planosContratados.map((plano, index) => (
                    ...
                    <React.Fragment key={plano.id_relacao}>
                        ...
                        {/* Cria um quarto cartão para mostrar as ações relacionadas ao plano */}
