// Importa os componentes necessários para a criação do diálogo de ações de internet
import {
	AlertDialog,
	AlertDialogAction,
	AlertDialogCancel,
...
// Importa a tipagem Planos para definir o tipo de dados esperado
...
interface DialogAcoesInternetProps {
  plano: Plano;
}
...
const deletarPlanoContratado = async (plano: Plano) => {
  ...
  const planos = await del<Response>(`/servicos_contratados_internet/${plano.id_relacao}`, BaseUrlKey.LOAD_BALANCER_URL, localStorage.getItem('token') || "");
...
// Define o componente DialogAcoesInternet, que é responsável por exibir um diálogo de ações de internet
export default function DialogAcoesInternet({plano}: DialogAcoesInternetProps) {
	return (
